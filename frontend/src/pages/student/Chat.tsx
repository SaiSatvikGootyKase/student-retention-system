...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    getContacts(currentUser.userId)
      .then(users => setContacts(users.filter(u => u.id !== currentUser.userId && u.role === 'FACULTY')))
      .catch(() => {})
  ...
  useEffect(() => {
    ...
    const load = () => {
      getConversation(currentUser.userId, selected.id)
        .then(setMessages)
        .catch(() => {});
    };
    load();
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-bold text-brand-navy">Chat with Mentor</h1>
        <p className="text-sm text-slate-500 mt-1">Message your faculty mentors directly</p>
      </div>
      <div className="flex gap-4 h-[calc(100vh-280px)] min-h-[500px]">
        ...
        <div className="w-72 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-brand-navy">Faculty Mentors</span>
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-slate-50">
            {loadingContacts ? (
              <div className="flex items-center justify-center h-32"><Loader2 className="animate-spin text-brand-indigo" size={20} /></div>
            ) : contacts.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-8">No faculty contacts found</p>
            ) : contacts.map(c => {
              ...
              return (
                <button key={c.id} onClick={() => setSelected(c)}
                  ...
                  <div className="w-9 h-9 rounded-full bg-brand-indigo flex items-center justify-center text-white text-xs font-bold shrink-0">{initials}</div>
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-brand-navy truncate">{c.name}</p>
                    <p className="text-xs text-slate-400 truncate">{c.department || 'Faculty'}</p>
                  </div>
                </button>
              );
            })}
        ...
        <div className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
          {!selected ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
              ...
              <p className="font-semibold">Select a faculty mentor to start chatting</p>
            </div>
          ) : (
            <>
              <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-brand-indigo flex items-center justify-center text-white text-xs font-bold">
                  {selected.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                </div>
                <div>
                  <p className="font-bold text-brand-navy text-sm">{selected.name}</p>
                  <p className="text-xs text-brand-emerald font-medium">Faculty</p>
                </div>
              </div>
                  ...
                  return (
                    <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs lg:max-w-md px-4 py-2.5 rounded-2xl text-sm shadow-sm ${
                        isMe ? 'bg-brand-indigo text-white rounded-br-sm' : 'bg-slate-100 text-brand-navy rounded-bl-sm'}`}>
                        <p>{msg.message}</p>
