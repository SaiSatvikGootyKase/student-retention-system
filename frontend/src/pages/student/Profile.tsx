...
import { getStudentProfile, getUser, patchStudentDropoutProfile, updateUser } from '../../api';
import type { DropoutMlProfilePayload, User as UserType } from '../../api';
import DropoutMlProfileFormFields from '../../components/student/DropoutMlProfileFormFields';
import { dropoutFeaturesToPayload, emptyPayload } from '../../utils/dropoutProfilePayload';
...
export default function Profile() {
  ...
  const [mlForm, setMlForm] = useState<DropoutMlProfilePayload | null>(null);
  ...
  useEffect(() => {
    ...
    if (role === 'STUDENT') {
      Promise.all([getUser(uid), getStudentProfile(uid)])
        .then(([u, profile]) => {
          setUser(u);
          setForm({
            name: u.name || '',
          ...
          setMlForm(dropoutFeaturesToPayload(profile.dropoutMlFeatures, u.department || ''));
        })
        .catch(() => {
          getUser(uid)
            .then(u => {
              ...
              setMlForm(emptyPayload(u.department || ''));
            })
            .catch(() => {});
        })
  ...
  const mlUpdate = <K extends keyof DropoutMlProfilePayload>(key: K, value: DropoutMlProfilePayload[K]) => {
    setMlForm(prev => (prev ? { ...prev, [key]: value } : prev));
  };

  const mlNum = (key: keyof DropoutMlProfilePayload, v: string, min: number, max: number) => {
    const n = parseInt(v, 10);
    if (Number.isNaN(n) || !mlForm) return;
    mlUpdate(key, Math.min(max, Math.max(min, n)) as DropoutMlProfilePayload[typeof key]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isStudent && mlForm) {
        ...
        const payload = await enrichMlPayloadFromLms(mlForm, sid);
        await patchStudentDropoutProfile(currentUser.userId, payload);
        markDropoutProfileComplete();
      }
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
    } catch (e: unknown) {
      setMlError(e instanceof Error ? e.message : 'Save failed');
    } finally {
      setSaving(false);
    }
  };
  ...
  const roleLabel = user?.role?.toLowerCase() || 'student';
        ...
        <div className="px-6 sm:px-10 pt-8 pb-10">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-10">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-slate-500 capitalize">{roleLabel}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
            ...
            <div className="space-y-1 min-w-0">
              <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Department / school</label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. Computer Science or CSIT"
                  className="w-full min-w-0 pl-9 pr-4 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-indigo/20 focus:border-brand-indigo bg-slate-50"
                />
              </div>
            </div>
          ...
          {isStudent && mlForm && (
            <div className="mt-10 pt-8 border-t border-slate-200 space-y-4">
              <div>
                <h3 className="text-lg font-bold text-brand-navy">Retention & dropout model</h3>
                <p className="text-sm text-slate-500 mt-1">
                  Same fields as the training dataset (labels adjusted for clarity). Failures and absences are filled
                  from your <strong>Results</strong> and <strong>Attendance</strong> when you save. Data is stored on
                  your profile and mirrored to <span className="font-mono text-slate-600">student_dropouts</span> in
                  the <span className="font-mono text-slate-600">student</span> database.
                </p>
              </div>
              {mlError && <p className="text-sm text-brand-rose font-medium">{mlError}</p>}
              <div className="space-y-8 max-h-[65vh] overflow-y-auto pr-1">
                <DropoutMlProfileFormFields form={mlForm} update={mlUpdate} num={mlNum} selectClass={selectClass} />
              </div>
            </div>
          )}
